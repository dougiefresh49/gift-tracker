...
export function StockView({
  ...
  const [filterReturnStatus, setFilterReturnStatus] = useState<ReturnStatusFilter>("none");
  ...
  const groupedGifts = useMemo(() => {
    ...
    if (groupBy === "recipient") {
      // Group by recipient - a gift can have multiple recipients
  ...
  const getGroupByLabel = (option: GroupByOption) => {
    switch (option) {
      ...
      case "recipient": return "By Recipient";
      case "gifter": return "By Gifter";
    }
  };
  ...
  const getSortLabel = (sort: SortOption) => {
    switch (sort) {
      ...
      case "price-asc": return "$ Low";
      case "price-desc": return "$ High";
    }
  };
  ...
  return (
    <div className="space-y-2">
          ...
          <div className="flex gap-1.5 items-center pb-2 min-w-max">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Badge 
                  variant={filterStatus !== "available" ? "secondary" : "outline"} 
                  className="cursor-pointer whitespace-nowrap text-xs py-1 gap-1"
                >
                  {filterStatus === "all" ? "All" : filterStatus === "claimed" ? "Claimed" : "Available"}
                </Badge>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" sideOffset={4}>
                <DropdownMenuItem onClick={() => setFilterStatus("available")}>Available</DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterStatus("claimed")}>Claimed</DropdownMenuItem>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  {filterRecipient === "all" 
                    ? "For Anyone" 
                    : profiles.find(p => p.id === filterRecipient)?.name ?? "For..."}
                </Badge>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" sideOffset={4}>
                <DropdownMenuItem onClick={() => setFilterRecipient("all")}>Anyone</DropdownMenuItem>
                <DropdownMenuSeparator />
                {profiles.map((p) => (
                  <DropdownMenuItem key={p.id} onClick={() => setFilterRecipient(p.id)}>
                    {p.name}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            ...
            <Sheet>
              ...
              <SheetContent side="bottom" className="rounded-t-xl max-h-[60vh] flex flex-col pb-20">
                ...
                <div className="overflow-y-auto flex-1 py-4">
                  <div className="grid gap-4">
                    {/* Gifter filter */}
                    <div>
                      <label className="text-sm font-medium mb-2 block">Claimed by</label>
                      <div className="flex flex-wrap gap-2">
                        <Badge
                          variant={filterGifter === "all" ? "default" : "outline"}
                        ...
                        <Badge
                          variant={filterGifter === "unclaimed" ? "default" : "outline"}
                          className="cursor-pointer"
                        ...
                        {profiles.map((p) => (
                          <Badge
                            key={p.id}
                            variant={filterGifter === p.id ? "default" : "outline"}
                            className="cursor-pointer"
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Badge
                          variant={filterReturnStatus === "none" ? "default" : "outline"}
                          className="cursor-pointer"
                        ...
                        <Badge
                          variant={filterReturnStatus === "all" ? "default" : "outline"}
                          className="cursor-pointer"
                        ...
                        <Badge
                          variant={filterReturnStatus === "to_return" ? "default" : "outline"}
                          className="cursor-pointer"
                        ...
                        <Badge
                          variant={filterReturnStatus === "returned" ? "default" : "outline"}
                          className="cursor-pointer"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Badge variant="outline" className="cursor-pointer whitespace-nowrap text-xs py-1 gap-1">
              ...
              <DropdownMenuContent align="end" sideOffset={4}>
                ...
                <DropdownMenuItem onClick={() => setSortOption("price-asc")}>Price Low → High</DropdownMenuItem>
                <DropdownMenuItem onClick={() => setSortOption("price-desc")}>Price High → Low</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" sideOffset={4}>
                ...
                <DropdownMenuItem onClick={() => setGroupBy("recipient")}>By Recipient</DropdownMenuItem>
                <DropdownMenuItem onClick={() => setGroupBy("gifter")}>By Gifter</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <div className="text-xs text-muted-foreground px-1">
        ...
        {groupBy !== "none" && groupedGifts && ` in ${groupedGifts.length} ${groupedGifts.length === 1 ? 'group' : 'groups'}`}
      </div>
